...
namespace nfx::datatypes::test
{
	...
	TEST( Int128Construction, ConstructionFromInt64Negative )
	{
		...
		EXPECT_EQ( static_cast<std::uint64_t>( value ), num.toLow() );
		EXPECT_EQ( constants::INT_128_MAX_POSITIVE_LOW, num.toHigh() ); // Sign extension
	}
	...
	TEST( Int128Assignment, UnaryMinus )
	{
		...
		EXPECT_TRUE( largeNeg.isNegative() );
	...
	TEST( Int128Arithmetic, AdditionWithCarry )
	{
		...
		datatypes::Int128 a{ constants::INT_128_MAX_POSITIVE_LOW, 0 };
	...
	TEST( Int128Arithmetic, SubtractionWithBorrow )
	{
		...
		EXPECT_EQ( constants::INT_128_MAX_POSITIVE_LOW, result.toLow() );
		EXPECT_EQ( 0ULL, result.toHigh() );
	}
	...
	TEST( Int128Arithmetic, MultiplicationLarge )
	{
		...
		EXPECT_EQ( ( a_val << 32 ) & constants::INT_128_MAX_POSITIVE_LOW, result.toLow() );
	...
	TEST( Int128Arithmetic, UnaryMinus )
	{
		...
		EXPECT_EQ( constants::INT_128_MAX_POSITIVE_LOW, negative.toHigh() );
	}
	...
	TEST( Int128StateChecking, IsNegativeHighBit )
	{
		...
		datatypes::Int128 negative{ 0, constants::INT_128_MIN_NEGATIVE_HIGH };
		datatypes::Int128 positive{ 0, constants::INT_128_MAX_POSITIVE_HIGH };
	...
	TEST( Int128EdgeCaseAndOverflow, MaxValues )
	{
		...
		datatypes::Int128 maxVal{ constants::INT_128_MAX_POSITIVE_LOW, constants::INT_128_MAX_POSITIVE_HIGH };

		EXPECT_FALSE( maxVal.isZero() );
		EXPECT_FALSE( maxVal.isNegative() );
		EXPECT_EQ( constants::INT_128_MAX_POSITIVE_LOW, maxVal.toLow() );
		EXPECT_EQ( constants::INT_128_MAX_POSITIVE_HIGH, maxVal.toHigh() );
	}

	TEST( Int128EdgeCaseAndOverflow, MinValue )
	{
		// Test with minimum possible value (most negative)
		datatypes::Int128 minVal{ 0, constants::INT_128_MIN_NEGATIVE_HIGH };

		EXPECT_FALSE( minVal.isZero() );
		EXPECT_TRUE( minVal.isNegative() );
		EXPECT_EQ( 0ULL, minVal.toLow() );
		EXPECT_EQ( constants::INT_128_MIN_NEGATIVE_HIGH, minVal.toHigh() );
	}

	TEST( Int128EdgeCaseAndOverflow, OverflowAddition )
	{
		// Test addition overflow
		datatypes::Int128 maxPositive{ constants::INT_128_MAX_POSITIVE_LOW, constants::INT_128_MAX_POSITIVE_HIGH };
		...
		EXPECT_TRUE( result.isNegative() );
		EXPECT_EQ( 0ULL, result.toLow() );
		EXPECT_EQ( constants::INT_128_MIN_NEGATIVE_HIGH, result.toHigh() );
	}

	TEST( Int128EdgeCaseAndOverflow, OverflowSubtraction )
	{
		// Test subtraction underflow
		datatypes::Int128 minNegative{ 0, constants::INT_128_MIN_NEGATIVE_HIGH };
		datatypes::Int128 one{ 1 };
		...
		EXPECT_FALSE( result.isNegative() );
		EXPECT_EQ( constants::INT_128_MAX_POSITIVE_LOW, result.toLow() );
		EXPECT_EQ( constants::INT_128_MAX_POSITIVE_HIGH, result.toHigh() );
	}

	TEST( Int128EdgeCaseAndOverflow, OverflowMultiplication )
	{
		// Test multiplication that overflows 128 bits
		datatypes::Int128 large1{ constants::INT_128_MAX_POSITIVE_LOW, constants::INT_128_MAX_POSITIVE_HIGH };
		...
		EXPECT_EQ( constants::INT_128_MAX_POSITIVE_LOW, result.toHigh() );
	}
	...
	TEST( Int128EdgeCaseAndOverflow, DivisionLargeNumbers )
	{
		...
		// Should be 2^63
		EXPECT_EQ( constants::INT_128_MIN_NEGATIVE_HIGH, result.toLow() );
		EXPECT_EQ( 0ULL, result.toHigh() );
	}
	...
	TEST( Int128EdgeCaseAndOverflow, ModuloLargeNumbers )
	{
		...
		datatypes::Int128 dividend{ constants::INT_128_MAX_POSITIVE_LOW, 1 }; // 2^64 + (2^64-1)
	...
	TEST( Int128EdgeCaseAndOverflow, SignBitBoundary )
	{
		...
		datatypes::Int128 justPositive{ constants::INT_128_MAX_POSITIVE_LOW, constants::INT_128_MAX_POSITIVE_HIGH };
		datatypes::Int128 justNegative{ 0, constants::INT_128_MIN_NEGATIVE_HIGH };
	...
	TEST( Int128EdgeCaseAndOverflow, ComparisonHighWordEdges )
	{
		...
		datatypes::Int128 a{ constants::INT_128_MAX_POSITIVE_LOW, constants::INT_128_MAX_POSITIVE_HIGH };
		datatypes::Int128 b{ 0, constants::INT_128_MIN_NEGATIVE_HIGH };
